...
for file in $files; do
    ...
    python3 -u load_tweets.py --db=postgresql://postgres:pass@localhost:1290/ "--inputs=$file"
done

echo 'load denormalized'
for file in $files; do
    # use SQL's COPY command to load data into pg_denormalized
    unzip -p "$file" | sed 's/\\u0000//g' | psql postgresql://postgres:pass@localhost:1289 / -c "COPY tweets_jsonb (data) FROM STDIN csv quote e'\x01' delimiter e'\x02';"
done
